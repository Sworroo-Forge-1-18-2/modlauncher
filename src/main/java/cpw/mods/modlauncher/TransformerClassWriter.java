...
class TransformerClassWriter extends ClassWriter {
    ...
    private void computeHierarchy(final String className) {
        ...
        if (clz == null && (className.startsWith("ru/cristalix/launcher/") || className.startsWith("com/mojang/authlib/") || className.startsWith("com/google"))) {
            try {
                clz = ClassLoader.getSystemClassLoader().loadClass(className.replace('/', '.'));
